...
bool Sphere::intersect(const Ray& worldRay,
                       ...
                       Hit& hit) const
{
    // --- 1) convert world ray (double) → float
    f3 rof( (float)worldRay.origin.x,
            (float)worldRay.origin.y,
            (float)worldRay.origin.z );
    f3 rdf( (float)worldRay.dir.x,
    ...
    // sphere data in float
    f3 posf(m_px, m_py, m_pz);
    f3 scalef(m_sx, m_sy, m_sz);

    // --- 2) float ellipsoid trick
    f3 oc = (rof - posf).cdiv(scalef);  // (ray.origin - position) / scale
    f3 dir = rdf.cdiv(scalef);          // ray.dir / scale
    ...
    float t_obj = std::numeric_limits<float>::max();
    ...
    if (t_obj == std::numeric_limits<float>::max())
        return false;

    // --- 3) float hit + float normal for ellipsoid
    f3 p_obj = oc + dir * t_obj;

    // gradient for ellipsoid → (x/sx^2, y/sy^2, z/sz^2)
    f3 scale_sq = scalef.cmul(scalef);
    f3 n_obj_f( p_obj.x / scale_sq.x,
                p_obj.y / scale_sq.y,
                p_obj.z / scale_sq.z );
    ...
    // --- 4) convert back to your engine types (double)
    Vec3 oc_d   = Vec3( (double)oc.x,     (double)oc.y,     (double)oc.z );
    Vec3 dir_d  = Vec3( (double)dir.x,    (double)dir.y,    (double)dir.z );
    Vec3 n_obj_d= Vec3( (double)n_obj_f.x,(double)n_obj_f.y,(double)n_obj_f.z );

    Ray objRay;
    objRay.origin = oc_d;
    objRay.dir    = dir_d;

    // t_obj is float, commit wants double
    double t_obj_d = (double)t_obj;

    // --- 5) let the base class finalize (does tMin/tMax, world transform, etc.)
    return commitFromObjectSpace(
        worldRay,
        objRay,
        t_obj_d,
