...
public:
    ...
    // Load first camera found in the JSON file
    void loadFromSceneTxt(const std::string& path);

    // Make a ray from pixel coords (float), origin at camera position.
    // px in [0, film_w_px), py in [0, film_h_px), py downwards as image row index.
    Ray rayFromPixel(float px, float py) const;

    void buildBasis();

private:
    // --- tiny, very permissive JSON helpers (std-only) ---
    static std::string readFile(const std::string& path);
    static bool findAfter(const std::string& s, const std::string& key, size_t& pos);
    static void skipWS(const std::string& s, size_t& i);
    static double parseNumber(const std::string& s, size_t& i);
    static std::vector<double> parseArrayN(const std::string& s, size_t& i, int N);
